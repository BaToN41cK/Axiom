...
## CLI-переопределения на один запуск

Флаги CLI меняют конфигурацию только для текущего запуска (файл не пишется):

```bash
axiom --ollama-url http://192.168.1.5:11434 -p "привет"
axiom -m gemma4:12b -p "..."
axiom --think off -p "..."
axiom --no-search -p "..."
```
